# -*- coding: utf-8 -*-
"""
Created on Mon Feb 26 20:03:30 2024
...
def pixel_to_degree(pixel_distance):
    # Piksel aralığını (20, 620) den (0, 180) arasına dönüştür
    min_pixel = 10
    max_pixel = 230
    min_degree = 0
    max_degree = 90
    ...
    degree = ((pixel_distance - min_pixel) / (max_pixel - min_pixel)) * (max_degree - min_degree) + min_degree
    if degree >90:
        degree = 90
    return int(degree)  # Servo hareket açısını tam sayı olarak döndür
...
def main():
    ...
    while True:
        ...
        if hands:
            ...
            servo_angle = pixel_to_degree(length) // 2
            print("############Servo Açısı:", servo_angle)
            move_servo(6,servo_angle)
            time.sleep(0.15)
            move_servo(5,45 -servo_angle)
            time.sleep(0.15)
            print("--------Parmaklar Arası UZaklık :",int(length))
...
# Örnek kullanım
pixel_distance = 620  # Örnek piksel uzaklığı
servo_angle = pixel_to_degree(pixel_distance)
print("Servo Açısı:", servo_angle)
